...
std::map<std::string, tocr_result> ocr(const config& app_cfg, display& disp, const surface& surf, const std::vector<std::string>& fields, const std::string& pb_short_path, const std::string& sha256, const tflite::tparams& tflite_params, const bool bg_color_partition);

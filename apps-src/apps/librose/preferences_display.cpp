...
void set_fullscreen(display& disp, const bool ison)
{
	...
	const std::pair<int,int>& res = resolution(disp.video());
...
bool show_resolution_dialog(display& disp)
{
	CVideo& video = disp.video();
...
{
	int res = instance->show_preferences_dialog(disp);
	if (res == preferences::CHANGE_RESOLUTION) {
		preferences::show_resolution_dialog(disp);

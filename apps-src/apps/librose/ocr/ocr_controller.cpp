...
void ocr_controller::recognize()
{
	...
	gui2::run_with_progress(*gui_, null_str, boost::bind(&ocr_controller::did_recognize, this, _1), true, 0, false);
...
std::map<std::string, tocr_result> ocr(const config& app_cfg, display& disp, const surface& surf, const std::vector<std::string>& fields, const std::string& tflite, const std::string& sha256, const tflite::tparams& tflite_params, const bool bg_color_partition)
{
	...
	ocr_controller controller(app_cfg, disp.video(), target, lines, fields, tflite, sha256, tflite_params);

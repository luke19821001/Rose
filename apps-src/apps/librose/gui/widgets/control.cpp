...
tpoint tcontrol::best_size_from_builder() const
{
	...
	if (best_width_.has_formula2()) {
		...
		if (best_width_.has_multi_or_noninteger_formula()) {
	...
	if (best_height_.has_formula2()) {
		...
		if (best_height_.has_multi_or_noninteger_formula()) {
...
void tcontrol::set_best_size(const std::string& width, bool width_is_max, const std::string& height, bool height_is_max)
{
	if (width_is_max) {
...
{
	VALIDATE(!best_width_.has_formula(), null_str);
	VALIDATE(!best_height_.has_formula(), null_str);
...
void tcontrol::set_label(const std::string& label)
{
	...
	if (require_invalidate_layout) {
		VALIDATE(this != window, null_str);
		twidget* tmp = this->parent();
		while (tmp) {

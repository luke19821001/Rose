...
void tmenu::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	window.set_border("menu-border");
	window.set_margin(4 * twidget::hdpi_scale, 4 * twidget::hdpi_scale, 4 * twidget::hdpi_scale, 4 * twidget::hdpi_scale);
...
void tmenu::item_focus_changed(twindow& window, tlistbox& list, twidget& panel, const bool enter)
{
	...
	if (!item.submenu.empty()) {
		...
		gui2::tmenu dlg(disp_, item.submenu, nposm, this);

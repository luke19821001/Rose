...
bool twindow_setting::save_base(twindow& window)
{
	...
	if (cell_.window.scene) {
		...
		if (!cell_.window.automatic_placement) {
			...
			if (!cell_.window.x.empty() && !verify_formula(disp_, "x's formula", cell_.window.x)) {
				return false;
			}
			text_box = find_widget<ttext_box>(&window, "y", false, true);
			cell_.window.y = text_box->label();
			utils::lowercase2(cell_.window.y);
			if (!cell_.window.y.empty() && !verify_formula(disp_, "y's formula", cell_.window.y)) {
				return false;
			}
			...
			if (!cell_.window.width.empty() && !verify_formula(disp_, "width's formula", cell_.window.width)) {
				return false;
			}
			...
			if (!cell_.window.height.empty() && !verify_formula(disp_, "height's formula", cell_.window.height)) {
				return false;
			}
		}
	}
...
bool twindow_setting::save_float_widget(twindow& window, const int at)
{
	...
	if (!item.w.first.empty()) {
		...
		if (!verify_formula(disp_, "width's fomula", item.w.first)) {
			return false;
		}
	...
	if (!item.h.first.empty()) {
		...
		if (!verify_formula(disp_, "height's fomula", item.h.first)) {
			return false;
		}

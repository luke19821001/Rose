...
void tcontrol_setting::post_show(twindow& window)
{
...
bool tcontrol_setting::save_base(twindow& window)
{
	...
	if (!cell_.id.empty()) {
		if (!utils::isvalid_id(cell_.id, false, min_id_len, max_id_len)) {
			gui2::show_message(disp_.video(), "", _("id error"));
			return false;
		}
	}
...
bool tcontrol_setting::save_size(twindow& window)
{
	...
	if (u_.has_size()) {
		...
		if (!cell_.widget.min_text_width.empty() && !verify_formula(disp_, "min_text_width's formula", cell_.widget.min_text_width)) {
			return false;
		}
	}
	if (u_.has_mtwusc()) {
...
static bool save_margin(display& disp, tgrid& grid, const std::string& id, int& ret)
{
	...
	if (!utils::isinteger(label)) {
		show_id_error(disp, id, _("must be integer."));
		return false;
	}

	ret = utils::to_int(label);
	if (ret < 0) {
		show_id_error(disp, id, _("must be >= 0."));
		return false;
	}
	return true;
}
...
bool tcontrol_setting::save_advanced(twindow& window)
{
	...
	if (u_.has_margin()) {
		if (!save_margin(disp_, *advanced_page, "_left_margin", cell_.widget.margin.left)) {
			return false;
		}
		if (!save_margin(disp_, *advanced_page, "_right_margin", cell_.widget.margin.right)) {
			return false;
		}
		if (!save_margin(disp_, *advanced_page, "_top_margin", cell_.widget.margin.top)) {
			return false;
		}
		if (!save_margin(disp_, *advanced_page, "_bottom_margin", cell_.widget.margin.bottom)) {
			return false;
		}
	}

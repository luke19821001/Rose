...
void tnew_tpl_widget::pre_show(CVideo& video, twindow& window)
{
	find_widget<tlabel>(&window, "title", false).set_label(_("Create or modify template widget"));
	...
	text_box->set_did_text_changed(boost::bind(&tnew_tpl_widget::did_id_changed, this, boost::ref(window), _1));
...
void tnew_tpl_widget::post_show(twindow& window)
{
	treport* options = find_widget<treport>(window_, "options", false, true);

...
tanim3::tanim3(display& disp, const config& app_cfg)
	: disp_(disp)
	, app_cfg_(app_cfg)
...
void tanim3::pre_show(CVideo& video, twindow& window)
{
	window.set_margin(8 * twidget::hdpi_scale, 8 * twidget::hdpi_scale, 0, 8 * twidget::hdpi_scale);
	window.set_label("misc/white-background.png");

	find_widget<tlabel>(&window, "title", false).set_label(_("Animation editor"));
	...
	connect_signal_mouse_left_click(
			  find_widget<tcontrol>(&window, "build", false)
			, boost::bind(
				&tanim3::do_build
				, this));

	connect_signal_mouse_left_click(
			  find_widget<tcontrol>(&window, "insert", false)
			, boost::bind(
	...
	connect_signal_mouse_left_click(
			  find_widget<tcontrol>(&window, "erase", false)
			, boost::bind(
...
void tanim3::did_double_click_editor(ttree_node& widget)
{
	...
	if (cookie == tanim2::PARAM_MODE) {
		...
		gui2::tanim_filter dlg(disp_, anim);
		dlg.show(disp_.video());

	} else if (tvi_is_frame(parent_cookie)) {
		gui2::tanim_frame dlg(disp_, anim, parent_cookie - tanim2::PARAM_FRAME);
		dlg.show(disp_.video());
...
void tanim3::did_right_button_up_editor(ttree_node& widget, const tpoint& coordinate)
{
	...
	if (cookie == tanim2::PARAM_ANIM) {
		...
		int selected;
		{
			...
			gui2::tmenu dlg(disp, items, nposm);
		...
		if (selected == add_frame_front) {
			...
			gui2::tanim_frame dlg(disp_, anim, cookie - tanim2::PARAM_FRAME);
			dlg.show(disp_.video());
...
void tanim3::erase_animation(twindow& window)
{
	...
	tanim2& anim = anims_updating_[clicked_anim_];
	int res = gui2::show_message(disp_.video(), "", _("Do you want to erase this animation?"), gui2::tmessage::yes_no_buttons);
...
void tanim3::do_erase_particular(const int at)
{
	...
	const int res = gui2::show_message(disp_.video(), _("Confirm delete"), ss.str(), gui2::tmessage::yes_no_buttons);

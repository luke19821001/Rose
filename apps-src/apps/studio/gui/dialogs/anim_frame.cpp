...
tanim_frame::tanim_frame(display& disp, tanim2& anim, const int at)
	: disp_(disp)
	, anim_(anim)
	, at_(at)
{
...
void tanim_frame::pre_show(CVideo& video, twindow& window)
{
	find_widget<tlabel>(&window, "title", false).set_label(_("anim^Edit frame"));

	variables_to_lv(window, anim_.id_);
	std::pair<tparticular*, int>& cookie = anim_.frame_cookie_[at_];
	cookie.first->update_to_ui_frame_edit(window, cookie.second);

	tstack* image_halo = find_widget<tstack>(&window, "image_halo", false, true);
	image_halo->set_radio_layer(cookie.first->use_image(cookie.second)? image_layer: halo_layer);

	ttoggle_button* use_image = find_widget<ttoggle_button>(&window, "use_image", false, true);
